 ...
 * @return  array   配送方式数组
 * @return  int   是否只显示默认配送方式
 */
function available_shipping_list($region_id_list,$suppid=0,$is_default_show=1,$shipping_code=null)
{
	$where = ' AND s.supplier_id='.$suppid.' and s.is_default_show=' . $is_default_show . ' and r.region_id ' . db_create_in($region_id_list);
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
    ...
    $cost_price             = attr_cost_price($spec);
    ...
    $parent = array(
		...
		'split_money'	=> get_split_money_by_id($goods_id),
        'goods_attr'    => addslashes($goods_attr),
        'goods_attr_id' => $goods_attr_id,
		'cost_price'	=> $goods['cost_price']+$cost_price,//代码增加--cb--推荐分成
    ...
    if ($num > 0)
    {
        ...
        if($row) //如果购物车已经有此物品，则更新
        {
            ...
            if ($GLOBALS['_CFG']['use_storage'] == 0 || $num <= $goods_storage)
            {
				...
				//app一键购买
				$_SESSION['one_step_buy_rec_id'] = $row['rec_id'];
            }
            else
            {
        ...
        else //购物车没有此物品，则插入
        {
            ...
            $parent['parent_id']    = 0;
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart'), $parent, 'INSERT');
			$_SESSION['one_step_buy_rec_id'] = $GLOBALS['db']->insert_id();
        }
...
//获取商品分成金额
function get_split_money_by_id($goods_id)
{
	...
	if($rows)
	{
		...
		{
			 $sql = "SELECT shop_price FROM " . $GLOBALS['ecs']->table('goods') . " WHERE goods_id = '" . $rows['goods_id'] . "'";
