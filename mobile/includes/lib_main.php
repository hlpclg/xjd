...
{
    $sql = 'SELECT cat_name FROM ' . $GLOBALS['ecs']->table('ecsmart_article_cat') . " WHERE cat_id = '" . $id ."'";
...
function get_shop_help()
{
    $sql = 'SELECT c.cat_id, c.cat_name, c.sort_order, a.article_id, a.title, a.file_url, a.open_type ' .
            ...
            'LEFT JOIN ' .$GLOBALS['ecs']->table('ecsmart_article_cat'). ' AS c ' .
...
function article_categories_tree($cat_id = 0)
{
    if ($cat_id > 0)
    {
        $sql = 'SELECT parent_id FROM ' . $GLOBALS['ecs']->table('ecsmart_article_cat') . " WHERE cat_id = '$cat_id'";
    ...
    /*
     ...
     如果不是取当前分类及其下的子分类
    */
    $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('ecsmart_article_cat') . " WHERE parent_id = '$parent_id'";
    if ($GLOBALS['db']->getOne($sql))
    {
        /* 获取当前分类及其子分类 */
        $sql = 'SELECT a.cat_id, a.cat_name, a.sort_order AS parent_order, a.cat_id, ' .
                    'b.cat_id AS child_id, b.cat_name AS child_name, b.sort_order AS child_order ' .
                'FROM ' . $GLOBALS['ecs']->table('ecsmart_article_cat') . ' AS a ' .
                'LEFT JOIN ' . $GLOBALS['ecs']->table('ecsmart_article_cat') . ' AS b ON b.parent_id = a.cat_id ' .
...
function get_article_parent_cats($cat)
{
    ...
    $arr = $GLOBALS['db']->GetAll('SELECT cat_id, cat_name, parent_id FROM ' . $GLOBALS['ecs']->table('ecsmart_article_cat'));
