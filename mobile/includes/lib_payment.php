    ...
    if ($log_id > 0){
        ...
        if ($pay_log && $pay_log['is_paid'] == 0){
            ...
            if ($pay_log['order_type'] == PAY_ORDER){
                /* 取得订单信息 */
                $sql = 'SELECT order_id, user_id,supplier_id, order_sn, consignee, address, tel, shipping_id, extension_code, extension_id, goods_amount ' .'FROM ' . $GLOBALS['ecs']->table('order_info') ." WHERE order_id = '$pay_log[order_id]' OR parent_order_id = '$pay_log[order_id]' ";
				...
				foreach($orderinfo as $key => $order){
	                ...
	                $GLOBALS['db']->query($sql);
					$sql = 'UPDATE ' . $GLOBALS['ecs']->table('pay_log') ." SET is_paid = '1' WHERE order_id = '$order_id'";
					$GLOBALS['db']->query($sql);
	                /* 记录订单操作记录 */
	               ...
	               /* 如果需要，发短信 */  //jx
					...
					//	发送微信消息
					send_wecaht($supplier,$GLOBALS['_CFG']['sms_order_payed_tpl'],2);					
					//付款给客户发短信
