    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.*, COUNT(s.cat_id) AS has_children, COUNT(a.article_id) AS aricle_num ".
               ' FROM ' . $GLOBALS['ecs']->table('ecsmart_article_cat') . " AS c".
               " LEFT JOIN " . $GLOBALS['ecs']->table('ecsmart_article_cat') . " AS s ON s.parent_id=c.cat_id".
               " LEFT JOIN " . $GLOBALS['ecs']->table('ecsmart_article') . " AS a ON a.cat_id=c.cat_id".
               " GROUP BY c.cat_id ".
...
{
    $sql = "SELECT pg.goods_id, g.goods_name, pg.goods_number,g.goods_thumb, p.goods_attr, p.product_number, p.product_id
            FROM " . $GLOBALS['ecs']->table('package_goods') . " AS pg
	...
	foreach($supplier_ids as $val){
		...
		elseif ($position == 2){
			if ($info['sms_order_payed'] == '1' && $info['sms_shop_mobile'] != ''){
		    	...
		    	//$content1 = str_replace(array('ordersn','shopname'),array($supplierinfo[$val],$_CFG['shop_name']),$content);
		    	foreach($phones as $phone){
		    		$aa = sendSMS($phone,$content1);
					file_put_contents('./bbbb',$aa);
		    	}
		    }
		}
	}
 ...
 *
 */
function send_wecaht($supplierinfo='',$content='',$position=1){
	global $_CFG;
	if(empty($supplierinfo) || empty($position)){
		return;
	}
	...
	//	入驻商家发送信息
	foreach($supplier_ids as $val){
		$sql = "select user.fake_id as openid from ". $GLOBALS['ecs']->table('weixin_user'). " as user inner join ". $GLOBALS['ecs']->table('supplier')." as supplier on supplier.user_id = user.ecuid  where supplier.supplier_id=".$val;
		$info = $GLOBALS['db']->getRow($sql);
	
		if($info['openid']){
			$content1 = sprintf($content,$supplierinfo[$val],$_CFG['shop_name']);
			$ret = pushToUserMsgs($info['openid'],'text',array('text'=>$content1));
			if ($ret){
...
function pushToUserMsgs($fake_id,$type="text",$msg=array(),$sendtime=0){
