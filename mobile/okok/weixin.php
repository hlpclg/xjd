...
switch ($act){
	...
	case "news":
		...
		if($_POST){
			...
			$createtime = time();
			if($_POST['msgtype']==1){
				if(preg_match('/[^\d,]+/', $_POST['artid']) || empty($_POST['artid'])){
...
function qcode_list()
{
	...
	if ($result === false)
	{
		...
		if($filter['type'] == 1){
			...
			$tn = $GLOBALS['ecs']->table('ecsmart_article');
			...
			$leftJoin = " left join {$GLOBALS['ecs']->table('goods')} on " . $GLOBALS['ecs']->table('weixin_qcode') . ".content={$GLOBALS['ecs']->table('goods')}.goods_id ";
			$leftJoin .= " left join {$GLOBALS['ecs']->table('ecsmart_article')} on " . $GLOBALS['ecs']->table('weixin_qcode') . ".content={$GLOBALS['ecs']->table('ecsmart_article')}.article_id ";
			$items = $GLOBALS['ecs']->table('weixin_qcode') . ".*,IF({$GLOBALS['ecs']->table('weixin_qcode')}.type=1, {$GLOBALS['ecs']->table('goods')}.goods_name, {$GLOBALS['ecs']->table('ecsmart_article')}.title) as title ";
			...
			if ($filter['keywords'])
			{
				...
				if($key)
				{
					$ex_where .= " or {$GLOBALS['ecs']->table('goods')}.goods_name like '{$key}'";
					$ex_where .= " or {$GLOBALS['ecs']->table('ecsmart_article')}.title like '{$key}'";
				}
			}
			else
			{
				if($key)
				{
					$ex_where .= " where {$GLOBALS['ecs']->table('goods')}.goods_name like '{$key}'";
					$ex_where .= " or {$GLOBALS['ecs']->table('ecsmart_article')}.title like '{$key}'";
				}
			}
		}
...
function addNews(){
    ...
    if(!$_POST['article_id']){
		$sql = "INSERT INTO ".$GLOBALS['ecs']->table('ecsmart_article')."(title, cat_id, article_type, is_open, author, ".
		...
		$GLOBALS['db']->query("update ".$GLOBALS['ecs']->table('ecsmart_article')." set 
