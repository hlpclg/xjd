...
switch ($act){
	case "list"://list
		...
		if($_POST){
			...
			if($aid > 0){
				$ret = $db->query ( 
					"UPDATE  " . $GLOBALS['ecs']->table('weixin_act') . "  SET 
					`title`='$title',
					`content`='$content',
					`isopen`='$isopen',
					`type`='$type',
				...
				$ret = $db->query ( 
					"insert into  " . $GLOBALS['ecs']->table('weixin_act') . "  (title,content,isopen,type,tpl,overymd,num) 
			...
			$act = $db->getRow ( "SELECT * FROM  " . $GLOBALS['ecs']->table('weixin_act') . "  where aid=$aid" );
			$smarty->assign('action_link',  array('text' => "奖项管理", 'href'=>'weixin_egg.php?act=listall&aid='.$aid));
			$smarty->assign ( 'act', $act );
			$smarty->display ( 'weixin/act_show.html' );
			return;
		}
		$act = $db->getAll ( "SELECT * FROM  " . $GLOBALS['ecs']->table('weixin_act'));
		$smarty->assign ( 'actList', $act );
		$smarty->display ( 'weixin/act_list.html' );
		break;
	case "listall":
		$aid = intval($_GET['aid']);
		$actList = $db->getAll ( "SELECT * FROM  " . $GLOBALS['ecs']->table('weixin_actlist') . "  where aid=$aid" );
	...
	case "add"://add and edit
		...
		$num = intval($_POST ['num']);
		if($lid > 0){
			$actList = $db->getRow ( "SELECT * FROM  " . $GLOBALS['ecs']->table('weixin_actlist') . "  where lid=$lid" );
			$smarty->assign ( 'actList', $actList );
			$sql = "update ". $GLOBALS['ecs']->table('weixin_actlist') ."  set title='$title',randnum=$randnum,num=$num,isopen=$isopen,awardname='$awardname' where lid=$lid";
		}else{
			$sql = "insert into ". $GLOBALS['ecs']->table('weixin_actlist') ."  (title,randnum,isopen,num,aid,awardname) 
	...
	case "log":
		...
		$tag = $_GET['tag'];
		if($lid > 0 && $tag == 'send'){
			$ret = $db->query("update " . $GLOBALS['ecs']->table('weixin_actlog') . " set issend=1 where lid=$lid");
			$link [] = array ('href' => 'weixin_egg.php?act=log','text' => '获奖管理');
			sys_msg ( '处理成功', 0, $link );
		}
		else if($lid > 0 && $tag == 'delete')
		{
			$ret = $db->query("DELETE FROM ".$GLOBALS['ecs']->table('weixin_actlog')." where lid = '".$lid."'");
			$link [] = array ('href' => 'weixin_egg.php?act=log','text' => '获奖管理');
			sys_msg ( '处理成功', 0, $link );
		} 
