		...
		//if ($order['order_amount'] > 0)
	    {
	        ...
	        else
	        {
				...
				if($_CFG['sms_user_money_change'] == 1)
				{
					...
					$content = sprintf($_CFG['sms_use_balance_reduce_tpl'],local_date("Y-m-d H:i:s",gmtime()),$order['order_amount'],$users['user_money'],$_CFG['sms_sign']);
    ...
    foreach($order_info as $ok=>$order){
	    ...
	    if ($order['user_id'] > 0 && $order['surplus'] > 0)
	    {
			...
			if($_CFG['sms_user_money_change'] == 1)
			{
				$sql = "SELECT user_money,mobile_phone FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '" . $order['user_id'] . "'";
				$users = $GLOBALS['db']->getRow($sql);
				$content = sprintf($_CFG['sms_use_balance_reduce_tpl'],local_date("Y-m-d H:i:s",gmtime()),$order['surplus'],$users['user_money'],$_CFG['sms_sign']);
				if($users['mobile_phone'])
				{
					require_once (ROOT_PATH . 'sms/sms.php');
					sendSMS($users['mobile_phone'],$content);
...
//                    }
   ...
   clear_cart($flow_type,$id_ext);
	...
	//	发送微信消息
	send_wecaht($supplier_ids,$_CFG['sms_order_placed_tpl'],1);
...
elseif($_REQUEST['step'] == 'ok')
{
	...
	if($order){
		...
		$order = $db->getRow("SELECT * FROM ".$ecs->table('pay_log')." WHERE  order_id = '".trim($_GET['order_id'])."'");
		$order['order_sn'] = $order['order_id'];
	
		$all_order = $db->getAll("SELECT * FROM ".$ecs->table('order_info')." WHERE parent_order_id = ".trim($_GET['order_id']));
		$i = 0;
		$split_order = array();
		foreach($all_order as $key => $val){
			$order['pay_name'] = $val['pay_name'];
			$order['pay_id'] = $val['pay_id'];
			$split_order['list'][$key] = $val;
			$split_order['list'][$key]['order_amount_formated'] = price_format($val['order_amount'],false);
			$i++;
		}
		
		$split_order['sub_order_count'] = $i;
		$smarty->assign('split_order',      $split_order);
	} 
