...
function action_validate (){
	...
	{
		exit(json_encode(array(
			'error' => 1, 'content' => '验证类型不能为空', 'url' => 'url.php'
		)));
	}
...
function action_to_password_reset()
{
	...
	if($_SESSION['security_validate'] != true){
		show_message('非法操作！', '返回上一页', 'user.php?act=password_reset', 'info');
	}
...
function action_do_password_reset ()
{
	...
	if($_SESSION['security_validate'] != true){
		exit(json_encode(array(
			'error' => 1, 'content' => '非法操作', 'url' => 'user.php'
		)));
	}
...
function action_do_email_binding ()
{
	...
	{
		exit(json_encode(array(
			'error' => 2, 'content' => $_LANG['invalid_email_code'], 'url' => ''
		)));
	}
	else if($_POST['email'] != $email)
	{
		exit(json_encode(array(
			'error' => 3, 'content' => $_LANG['email_changed'], 'url' => ''
		)));
	}
	$result = validate_email_code($email, $email_code);
...
function action_email_validate (){
	...
	$email = $db->getOne($sql);
	if(empty($email)){
		show_message('您还未绑定邮箱地址！', array('去绑定邮箱', "返回账户安全中心"	), array('user.php?act=email_binding', 'user.php'), 'info');
	}
...
function action_to_mobile_binding ()
{
	...
	if($_SESSION['security_validate'] != true)
	{
		show_message('您还未绑定手机号码！', array(
			...
			'user.php?act=mobile_binding', 'user.php'
		), 'info');
	}
...
function action_mobile_validate ()
{
	...
	if(empty($mobile))
	{
		show_message('您还未绑定手机号，请先绑定！', '绑定手机号', 'user.php?act=mobile_binding', 'info');
	}
...
function action_send_mobile_code ()
{
	...
	// 短信内容
	$content = sprintf($_LANG['mobile_code_template'], $mobile_code, $GLOBALS['_CFG']['sms_sign']);
	...
	if($result)
	{
		...
		$_SESSION[VT_MOBILE_VALIDATE] = $mobile_phone;
		// 保存验证信息
		save_validate_record($mobile_phone, $mobile_code, VT_MOBILE_VALIDATE, time(), time() + 30 * 60);
		echo 'ok';
	}
	else
	{
