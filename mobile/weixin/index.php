...
if(!$followInfo or $followInfo['expire_in']-21000<time()){
...
if ($event['event'] == "SCAN"){
	...
	if($res){
		if($res['type'] == 1){
			...
			$artInfo = $db->getRow("select * from ".$GLOBALS['ecs']->table('ecsmart_article')." where article_id='{$res['content']}'");
			$newsData[0]['Title'] = $artInfo['title'];
