...
class weixinapi{
	...
	function getUserInfo($wxid){
		...
		function is_bind_distrib($wxid,$uid)
	{
		$sql = "select ecuid from ".$GLOBALS['ecs']->table('weixin_user')." where fake_id = '$wxid'";
		$ecuid = $GLOBALS['db']->getOne($sql);
		if($ecuid > 0)
		{
			$sql = "select parent_id from ".$GLOBALS['ecs']->table('users')." where user_id = '$ecuid'";
			...
			{
				$sql = "update ".$GLOBALS['ecs']->table('users')." set parent_id = '$uid' where user_id = '$ecuid'";
	...
	function bindUser($wxid,$email,$pwd,$username=''){
		...
		//修改新注册的用户成为普通分销商
		$GLOBALS['db']->query("UPDATE ".$GLOBALS['ecs']->table('users')." SET is_fenxiao = 2 WHERE user_id = '$user_id'");
	...
	**/
	...
	function createTokenLoginUrl($wxid,$dir=''){
		//define('TOKEN', "leileiceshi");//注意和redirect.php一直 可以移植到init.php中
		$t = time();
