    ...
    if ($is_insert) {
        if ($code == ''){
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral,sales_volume_base, suppliers_id, cost_price,wholesale_price)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral','$_POST[sales_volume_base]', '$suppliers_id', '$cost_price','$wholesale_price')";
        }
        else{
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, zhekou, promote_price, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,sales_volume_base, cost_price, wholesale_price)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral','$_POST[sales_volume_base]', '$cost_price', '$wholesale_price')";
        }
    }
    else{
        /* 如果有上传图片，删除原来的商品图 */
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
				...
				"sales_volume_base = '$_POST[sales_volume_base]', " .
                "is_best = '$is_best', " .
                "is_new = '$is_new', " .
                "is_hot = '$is_hot', " .
    ...
    if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list'])))
    {
        ...
        if(isset($_POST['attr_id_list']))
        {
            foreach ($_POST['attr_id_list'] AS $key => $attr_id){
				...
				$wholesale_price = $_POST['attr_cost_price'][$key];
				$attr_price = ($attr_price>=0) ? $attr_price : 0;
				
				$wholesale_price = ($attr_price > $wholesale_price) ? $wholesale_price : 0;
				$cost_price = ($attr_price > $wholesale_price) ? $attr_price-$wholesale_price : 0;
                ...
                if (!empty($attr_value))
                {
                    if (isset($goods_attr_list[$attr_id][$attr_value]))
                    {
                        ...
                        $goods_attr_list[$attr_id][$attr_value]['cost_price'] = $cost_price;
                        $goods_attr_list[$attr_id][$attr_value]['wholesale_price'] = $wholesale_price;
                    }
                    else
                    {
                        // 如果原来没有，标记为新增
        ...
        foreach ($goods_attr_list as $attr_id => $attr_value_list)
        {
            foreach ($attr_value_list as $attr_value => $info)
            {
                if ($info['sign'] == 'insert')
                {
                    $sql = "INSERT INTO " .$ecs->table('goods_attr'). " (attr_id, goods_id, attr_value, attr_price,cost_price,wholesale_price)".
                            "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]', '$info[cost_price]', '$info[wholesale_price]')";
                }
                elseif ($info['sign'] == 'update')
                {
                    $sql = "UPDATE " .$ecs->table('goods_attr'). " SET attr_price = '$info[attr_price]', cost_price = '$info[cost_price]', wholesale_price = '$info[wholesale_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                }
                else
                {
...
elseif ($_REQUEST['act'] == 'edit_goods_price')
{
	...
	$sql_zk = "SELECT promote_price,wholesale_price FROM " . $ecs->table('goods') . " WHERE goods_id = " . $goods_id;
	$goods_info = $db->getRow($sql_zk);
	$promote_price = $goods_info['promote_price'];
	$wholesale_price = $goods_info['wholesale_price'];
	if ($promote_price == 0){
		$zhekou = 10.0;
	}
	else{
    ...
    if ($goods_price < 0 || $goods_price == 0 && $_POST['val'] != "$goods_price"){
		...
		if($goods_price > 0 && $wholesale_price > 0){
			$cost_price = $goods_price - $wholesale_price;
			if($cost_price <= 0){
				make_json_error('修改失败，售价应大于供货价');
			}
		}else{
			$cost_price = 0;
		}
        if ($exc->edit("zhekou = '$zhekou', shop_price = '$goods_price', market_price = '$price_rate', cost_price = '$cost_price', last_update=" .gmtime(), $goods_id)){
...
elseif ($_REQUEST['act'] == 'product_list')
{
    ...
    $smarty->assign('attribute_count_3',        ($attribute_count + 5));
...
elseif ($_REQUEST['act'] == 'product_add_execute')
{
    ...
    foreach($product['product_sn'] as $key => $value)
    {
		...
		/* $sql = "SELECT sum(attr_price) as attr_price,sum(wholesale_price) as wholesale_price FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_id = '$product[goods_id]' AND goods_attr_id in (".implode(',',$goods_attr_id).") ";
