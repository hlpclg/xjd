...
if ($_REQUEST['act']== 'view')
{
	 ...
	 else
	{
		...
		if($rebate['status']>0){
			...
			// 计算佣金
			$back_and = '';
			if(($back_order_id = getBackOrderByRebate($id)) != false){
				//获取退货订单中相关订单
			...
			$money_info['online']['rebatemoney'] = price_format($online_cpirce);
			$money_info['onout']['allmoney'] = price_format($onout);
			$money_info['onout']['rebatemoney'] = price_format($onout_cprice);
...
function getOkOrder(){
	...
	while($row = $db->fetchRow($query)){
		...
		//$row['formated_rebate_fee'] = 0-price_format($row['total_fee']*$rebate['supplier_rebate']/100);			// 根据分成比例计算 订单分成金额
		$row['formated_rebate_fee'] = 0-price_format($cost_price);							// 根据商品设置分成金额  计算订单分成金额
	...
	//var_dump($row);die;
        ...
        if($is_order && $is_shipping && $is_pay){
        	...
        	$row['datas'] = $GLOBALS['_CFG']['tuihuan_days_qianshou'] - $cha ;
			if($row['datas'] <= 0){
				$row['is_rebeat'] = 1;
			}
        }
...
function changeStatus($rid){
	...
	$sql = "select oi.order_id,oi.order_status,oi.shipping_status,oi.pay_status,IFNULL(bo.back_type,'-1') as backtype from ".$ecs->table('order_info')." as oi left join ".$ecs->table('back_order')." as bo on oi.order_id=bo.order_id and bo.status_back < 6 where oi.rebate_id=".$rid." and oi.rebate_ispay=1";
	$info = $db->getAll($sql);
	if(empty($info)){
		//全部订单已经返佣
		return true;
