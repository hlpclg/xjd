...
elseif ($_REQUEST['act'] == 'upload')
{
    ...
    $arr = array();
    $goods_list = array();
	unset($_LANG['upload_goods']['integral']);
	unset($_LANG['upload_goods']['is_best']);
	unset($_LANG['upload_goods']['is_new']);
	unset($_LANG['upload_goods']['is_hot']);
	unset($_LANG['upload_goods']['is_on_sale']);
	unset($_LANG['upload_goods']['is_alone_sale']);
	unset($_LANG['upload_goods']['is_real']);
	...
	elseif($_POST['data_cat'] == 'taobao' || $_POST['data_cat'] == 'paipai' || $_POST['data_cat'] == 'paipai3' || $_POST['data_cat'] == 'taobao46'   ){
        $id_is = 0;
        foreach ($data AS $line){
            // 跳过第一行
            if ($line_number == 0){
            ...
            $arr['brand_name'] = $line_list[2];
            $arr['market_price'] = $line_list[3];
            $arr['shop_price'] = $line_list[4];
            $arr['original_img'] = $line_list[5];
            $arr['keywords'] = $line_list[8];
            $arr['goods_brief'] = strip_tags($line_list[9]);
            $arr['goods_desc'] = strip_tags($line_list[10]);
            $arr['goods_number'] = $line_list[12];
			$arr['warn_number'] = $line_list[13];
    ...
    // 显示的字段列表
    $smarty->assign('field_show', array('goods_name' => true, 'goods_sn' => true, 'brand_name' => true, 'market_price' => true, 'shop_price' => true, 'goods_number' => true));
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    if (isset($_POST['checked']))
    {
        ...
        foreach ($_POST['checked'] AS $key => $value)
        {
			...
			$field_arr['supplier_id'] = $_SESSION['supplier_id'];
	
            $db->autoExecute($ecs->table('goods'), $field_arr, 'INSERT');
			$max_id = $db->insert_id() + 1;
			$supplier_goods_cat['goods_id'] = $db->insert_id();
			$supplier_goods_cat['cat_id'] = $field_arr['cat_id'];
			$supplier_goods_cat['supplier_id'] = $_SESSION[supplier_id];
			$db->autoExecute($ecs->table('supplier_goods_cat'), $supplier_goods_cat, 'INSERT');			
