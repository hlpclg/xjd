...
elseif ($_REQUEST['act'] == 'view')
{
	...
	else
	{
		...
		$allmoney = $online+$onout;			// 总金额
		$all_cost_price = $online_cpirce+$onout_cprice;		// 返现总金额
		...
		$tongji['allrebate'] = price_format($all_cost_price);
		$tongji['chamoney'] = price_format($allmoney-$all_cost_price);
...
function rebate_list($act='')
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
	{
		...
		$info = $GLOBALS['db']->getRow("select sum(" . order_amount_field() . ") as all_money,(bonus+integral_money) as discount_pirce from ". $GLOBALS['ecs']->table('order_info') ." where rebate_id=". $row['rebate_id'] ." and rebate_ispay=2");
