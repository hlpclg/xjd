...
function delete_goods($goods_id)
{
    ...
    $res = $GLOBALS['db']->query($sql);
	  ...
	  */
    $sql = "SELECT img_url, thumb_url, img_original " .
            "FROM " . $GLOBALS['ecs']->table('bar_code') .
            " WHERE goods_id " . db_create_in($goods_id);
     $GLOBALS['db']->query($sql);
	/*删除条形码结束*/
    while ($row = $GLOBALS['db']->fetchRow($res))
...
function get_attr_list($cat_id, $goods_id = 0)
{
    ...
    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type,a.attr_txm, a.attr_values, v.attr_value, v.attr_price , v.cost_price  , v.wholesale_price ".
...
function build_attr_html($cat_id, $goods_id = 0 , $bar_code = 0)
{
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        $html .= ($val['attr_type'] == 1 || $val['attr_type'] == 2) ? $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]" value="' . $val['attr_price'] . '" size="5" maxlength="10" /> 属性供货价 <input type="text" name="attr_cost_price[]" value="' . $val['wholesale_price'] . '" size="5" maxlength="10" /> 供货价小于属性价格' :
            ' <input type="hidden" name="attr_price_list[]" value="0" /> <input type="hidden" name="attr_cost_price[]" value="0" />';
