...
.gallery_img_box{float:left;width:120px;border:1px solid #eeeeee;padding:10px;margin-right:5px;text-align:center;position:relative;}
.gallery_img{width:100px;height:100px;}
.blank{height:10px; line-height:10px; clear:both; visibility:hidden;}
.shuxingtupian{color:#fff;background:#ff3300;}

#box{ margin:0px 15px; width:715px; min-height:35px; background:#FF9; margin-top:10px;}
...
if ($img_list)
{
	foreach($img_list as $img)
	{
		...
		echo '<br><br><a href="?act=drop_image&goods_id=' . $goods_id . '&goods_attr_id=' . $goods_attr_id . '&img_id='. $img['img_id'] .'" onclick="javascript: return  (confirm(\'确认删除此图片吗\'))">[删除]</a> <a href="?act=set_attrimage&goods_id=' . $goods_id . '&goods_attr_id=' . $goods_attr_id . '&img_id='. $img['img_id'] .'">[属性图片]</a>';
		echo '</div>';
	}
}
else
